...
const Paketas = (props) => {
  ...
  const [pasirinktas, setPaketas] = useState(null);

  const pasirinkti = (variantas) => {
    props.pasirinkimas(paketai[variantas].kaina);
    setPaketas(variantas);
  };

  return (
    <div className="card">
      <h2>Pagrindinis paketas.</h2>
      <div className="options">
        {paketai.map((paketas, index) => (
          <div
            key={index}
            className={`option ${pasirinktas === index ? "selected" : ""}`}
            onClick={() => pasirinkti(index)}
          >
            <div className="title">{paketas.pavadinimas}</div>
            <div className="price">{paketas.kaina} &euro;</div>
            <svg
              width="20"
              height="21"
              viewBox="0 0 20 21"
