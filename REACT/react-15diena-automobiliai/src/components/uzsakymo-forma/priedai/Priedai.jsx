...
const Priedai = (props) => {
  const priedai = [
    {
      pavadinimas: "Stoglangis",
      kaina: 200,
    },
    {
      pavadinimas: "Parktronikai",
      kaina: 300,
    },
    {
      pavadinimas: "Galine vaizdo kamera",
      kaina: 250,
    },
  ];
  const [pasirinkimai, setPriedai] = useState([]);

  const pasirinkti = (variantas) => {
    let naujaReiksme = pasirinkimai;

    if (pasirinkimai.includes(variantas)) {
      naujaReiksme = pasirinkimai.filter((priedas) => variantas !== priedas);
    } else {
      naujaReiksme = [...naujaReiksme, variantas];
    }

    props.pasirinkimas(naujaReiksme.reduce((a, b) => a + priedai[b].kaina, 0));
    setPriedai(naujaReiksme);
  };

  return (
    <div className="card">
      <h2>Papildomi priedai.</h2>
      <div className="options">
        {priedai.map((priedas, index) => (
          <div
            key={index}
            className={`option ${
              pasirinkimai.includes(index) ? "selected" : ""
            }`}
            onClick={() => pasirinkti(index)}
          >
            <div className="title">{priedas.pavadinimas}</div>
            <div className="price">{priedas.kaina} &euro;</div>
            <svg
              width="20"
              height="21"
              viewBox="0 0 20 21"
