...
const Variklis = (props) => {
  const varikliai = [
    {
      pavadinimas: "Benzinas",
      kaina: 0,
    },
    {
      pavadinimas: "Dyzelinas",
      kaina: 2000,
    },
    {
      pavadinimas: "Elektra",
  ...
  const [pasirinktas, setVariklis] = useState(null);

  const pasirinkti = (variantas) => {
    props.pasirinkimas(varikliai[variantas].kaina);
    setVariklis(variantas);
  };
  ...
  return (
    <div className="card">
      ...
      <div className="options">
        {varikliai.map((variklis, index) => (
          <div
            key={index}
            className={`option ${pasirinktas === index ? "selected" : ""}`}
            onClick={() => pasirinkti(index)}
          >
            <div className="title">{variklis.pavadinimas}</div>
            <div className="price">+{variklis.kaina} &euro;</div>
            <svg
              width="20"
              height="21"
              viewBox="0 0 20 21"
