...
const Salonas = (props) => {
  ...
  const [pasirinktas, setSalonas] = useState(null);

  const pasirinkti = (variantas) => {
    props.pasirinkimas(salonai[variantas].kaina);
    setSalonas(variantas);
  };
  ...
  return (
    <div className="card">
      ...
      <div className="options">
        {salonai.map((salonas, index) => (
          <div
            key={index}
            className={`option ${pasirinktas === index ? "selected" : ""}`}
            onClick={() => pasirinkti(index)}
          >
            <div className="title">{salonas.pavadinimas}</div>
            <div className="price">{salonas.kaina} &euro;</div>
            <svg
              width="20"
              height="21"
              viewBox="0 0 20 21"
